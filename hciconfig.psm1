        ...
        $result = foreach ($p in $pnpAll) {
            ...
            foreach ($regPath in $regCandidates) {
                ...
                if (Test-Path $regPath) {
                    $lmpVersion    = (Get-ItemProperty $regPath -Name 'LmpVersion'    -ErrorAction SilentlyContinue)?.LmpVersion
                    $lmpSubVersion = (Get-ItemProperty $regPath -Name 'LmpSubversion' -ErrorAction SilentlyContinue)?.LmpSubversion
                }
            }
            ...
            if ($null -eq $lmpVersion -and $service) {   # FIX PSA: $null à gauche (PSPossibleIncorrectComparisonWithNull)
                ...
                if (Test-Path $btRegSvc) {
                    $lmpVersion    = (Get-ItemProperty $btRegSvc -Name 'LmpVersion'    -ErrorAction SilentlyContinue)?.LmpVersion
                    $lmpSubVersion = (Get-ItemProperty $btRegSvc -Name 'LmpSubversion' -ErrorAction SilentlyContinue)?.LmpSubversion
                }
            }
        ...
        foreach ($d in $devices) {
            ...
            if ($d.Service) {
                ...
                if ($svcWmi -and $svcWmi.PathName) { Write-Host "  SYS Path     : $($svcWmi.PathName)" }
                    ...
                    $sysFile = (Get-Item "$env:SystemRoot\System32\drivers\$($d.Service).sys" -ErrorAction SilentlyContinue)?.FullName
